...
class owoniverse(commands.Bot):
    ...
    async def on_ready(self):
        ...
        for i in await self.pool.fetch("SELECT * FROM settings"):
            self.guild_config[i["id"]] = {
                "prefix": i["prefix"],
                "log": {
                    "channel": i["log"],
                    "member_logs": i["member_logs"],
                    "mod_logs": i["mod_logs"],
                    "message_logs": i["message_logs"],
                    "guild_logs": i["guild_logs"],
                },
            }

        await self.change_presence(
            status=discord.Status.dnd, activity=discord.Game(f"I don't know."),
        )
    ...
    def add_to_cache(self, guild: int, logging: bool = False):
        if guild in self.guild_config:
            return

        self.guild_config[guild] = {
            "prefix": None,
            "log": {
                "channel": None,
                "member_logs": logging,
                "mod_logs": logging,
                "message_logs": logging,
                "guild_logs": logging,
            },
        }

    async def add_prefix(self, guild: int, prefix: str):
        self.add_to_cache(guild)

        self.guild_config[guild]["prefix"] = prefix
        await self.pool.execute(
            "INSERT INTO settings(id, prefix) VALUES ($1, $2) ON CONFLICT (id) DO UPDATE SET prefix = $2",
            guild,
            prefix,
        )
    ...
    async def remove_prefix(self, guild: int):
        ...
        await self.pool.execute(
            "UPDATE settings SET prefix = NULL WHERE id = $1", guild
        )

    async def add_log(self, guild: int, channel: int):
        self.add_to_cache(guild, True)

        self.guild_config[guild]["log"]["channel"] = channel
        await self.pool.execute(
            "INSERT INTO settings(id, log) VALUES ($1, $2) ON CONFLICT (id) DO UPDATE SET log = $2",
            guild,
            channel,
        )
    ...
    async def toggle_log(self, guild: int, log: str):
        ...
        await self.pool.execute(
            f"UPDATE settings SET {log} = $1 WHERE id = $2", not current, guild
        )
    ...
    async def add_case(self, guild: int, action: str, mod: int, user: int, reason: str):
        case = await self.pool.fetchval(
            'INSERT INTO cases(guild, action, mod, "user", reason) VALUES ($1, $2, $3, $4, $5) RETURNING id',
            guild,
            action,
            mod,
            user,
    ...
    async def get_case(self, guild: int, case: int):
        case = await self.pool.fetchrow(
            "SELECT * FROM cases WHERE guild = $1 AND id = $2", guild, case
        )
        return case

    async def get_all_cases(self, guild: int, limit: int, mod: int = None):
        if mod is None:
            case = await self.pool.fetch(
                "SELECT * FROM cases WHERE guild = $1 LIMIT $2", guild, limit
            )
        if mod is not None:
            case = await self.pool.fetch(
                "SELECT * FROM cases WHERE guild = $1 AND mod = $2 LIMIT $3",
                guild,
                mod,
                limit,
            )
    ...
    async def modify_case(
        self, guild: int, id: int, mod: int, owner: bool, reason: str
    ):
        if owner:
            case = await self.pool.fetchval(
                "UPDATE cases SET reason=$1 WHERE guild = $2 AND id = $3 RETURNING id",
                reason,
                guild,
                id,
            )
        if not owner:
            case = await self.pool.fetchval(
                "UPDATE cases SET reason=$1 WHERE guild = $2 AND id = $3 AND mod = $4 RETURNING id",
                reason,
                guild,
                id,
                mod,
